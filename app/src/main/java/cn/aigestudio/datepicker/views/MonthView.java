...
public class MonthView extends View {
    ...
    private void draw(Canvas canvas, Rect rect, DPInfo info) {
        ...
        String date = null;
        if(mDPDecor != null){
            date = centerYear + "-" + centerMonth + "-" + info.strG;
            date = mDPDecor.getBottomText(date);
        }
        if (date != null) drawFestival(canvas, rect, date, info.isFestival);
    ...
    private void drawGregorian(Canvas canvas, Rect rect, String str, DPInfo info) {
        ...
        if(mDPDecor != null){
            contains = mDPDecor.isContains(date);
        }
        if(contains){
            if (info.isWeekend || mDPDecor.isHoliday(date)) {
                mPaint.setColor(mTManager.colorF());
            } else {
                mPaint.setColor(mTManager.colorG());
            }
        } else {
            mPaint.setColor(mTManager.colorFB());
        }
    ...
    private void drawFestival(Canvas canvas, Rect rect, String str, boolean isFestival) {
     ...
     //   }
        if (str.contains("&")) {
            ...
            if(s.length == 2){
                String str1 = s[0];
                float strMeasure = mPaint.measureText(str1);
                if (strMeasure > rect.width()) {
                    float ch = mPaint.measureText(str1, 0, str1.length());
                            ...
                            animSet.addListener(new AnimatorListenerAdapter() {
                                ...
                                public void onAnimationEnd(Animator animation) {
                                    if (null != onDatePickedListener) {
                                        onDatePickedListener.onDatePicked(datetime);
                                    }
                                }
                            });
                            animSet.start();
