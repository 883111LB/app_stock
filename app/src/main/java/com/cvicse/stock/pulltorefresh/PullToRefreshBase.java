...
import com.cvicse.stock.pulltorefresh.internal.FlipLoadingLayout;
import com.cvicse.stock.pulltorefresh.internal.LoadingLayout;
import com.cvicse.stock.pulltorefresh.internal.RotateLoadingLayout;
import com.cvicse.stock.pulltorefresh.internal.Utils;
import com.cvicse.stock.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
				...
				if (isReadyForPull()) {
					...
					if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
						if (mMode.showHeaderLoadingLayout() && diff >= 1f && isReadyForPullStart()) {
							...
							if (mMode == Mode.BOTH || mMode == Mode.BOTH_NO_REFRESH) {
	...
	public final void onRefreshComplete() {
		long continued = System.currentTimeMillis() - refreshDown;
		if (isRefreshing()) {
			sharedPreferences.edit().putString(timeKey,Utils.timeFormat()).apply();
			handler.sendEmptyMessageDelayed(1,continued > 500 ? 0:500-continued);
		}
	}

	/*@Override
	public final void onRefreshDelayComplete() {
		long continued = System.currentTimeMillis() - refreshDown;
		if (isRefreshing()) {
			sharedPreferences.edit().putString(timeKey,Utils.timeFormat()).commit();
			handler.sendEmptyMessageDelayed(1,continued > 500 ? 0:1500);
		}
	}*/
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					if (mState == State.RELEASE_TO_REFRESH
							&& (null != mOnRefreshListener || null != mOnRefreshListener2) && mMode != Mode.BOTH_NO_REFRESH) {
	...
	protected void updateUIForMode() {
		...
		// set it to pull down
		mCurrentMode = (mMode == Mode.BOTH || mMode == Mode.BOTH_NO_REFRESH) ?  Mode.PULL_FROM_START : mMode;
	}
	...
	private String timeKey = "commmon_tag";
	...
	private void callRefreshListener() {
		refreshDown = System.currentTimeMillis();
	...
	private void init(Context context, AttributeSet attrs) {
		sharedPreferences = context.getSharedPreferences("ListView_time",Context.MODE_PRIVATE);
		switch (getPullToRefreshScrollDirection()) {
			case HORIZONTAL:
	...
	public static enum Mode {
		...
		boolean showHeaderView(){
			return this == BOTH_NO_REFRESH ? false:true;
		}
		...
		public boolean showHeaderLoadingLayout() {
			return this == PULL_FROM_START || this == BOTH || this == BOTH_NO_REFRESH;
		}
		...
		public boolean showFooterLoadingLayout() {
			return this == PULL_FROM_END || this == BOTH || this == MANUAL_REFRESH_ONLY || this == BOTH_NO_REFRESH;
		}
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				 ...
				 * smallMoneyInflow rounding errors
				 */
