package com.cvicse.stock.markets.ui.subnewbondstock;
...
import com.cvicse.stock.markets.ui.subnewbondstock.adapter.SubNewBondStockListAdapter;
 ...
 */
public class SubNewBondStockActivity extends PBaseActivity implements SubNewBondStockContract.View, AdapterView.OnItemClickListener {

    @MVPInject
    SubNewBondStockContract.Presenter presenter;
    ...
    private String[] titleNomalTextRaise = {"股票名称", "发行价", "最新价", "发行日期", "累计涨跌幅", "涨跌额", "成交额"};
    private String[] titleUpTextRaise = {"股票名称", "发行价↑", "最新价↑", "发行日期↑", "累计涨跌幅↑", "涨跌额↑", "成交额↑"};
    private String[] titleDownTextRaise = {"股票名称", "发行价↓", "最新价↓", "发行日期↓", "累计涨跌幅↓", "涨跌额↓", "成交额↓"};
    ...
    private String[] sortFields = {"0", "2", "3", "4", "7", "8", "10"};
    ...
    private SubNewBondStockListAdapter adapter;
    ...
    public static void actionStart(Context context) {
        Intent intent = new Intent(context, SubNewBondStockActivity.class);
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        mTextViews = new TextView[]{tev_name, tev_originalPrice,
                tev_lastestPrice, tev_originalData, tev_allRate, tev_change,
                tev_amount
        };
        ...
        adapter = new SubNewBondStockListAdapter(this);
        ...
        lsvContent.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener2<ListView>() {
            ...
            public void onPullDownToRefresh(PullToRefreshBase<ListView> refreshView) {
                ...
                if (currentPage == 0) {
                    // 如果页码等于0，直接刷新
                    presenter.subNewBondStockRanking(ADDFLAG_REFRESH, 0, 20, sortField, sortType);
                } else {
                    presenter.subNewBondStockRanking(ADDFLAG_REFRESH, (currentPage - 1) * 20, 20, sortField, sortType);
                }
            ...
            public void onPullUpToRefresh(PullToRefreshBase<ListView> refreshView) {
                // 上拉加载
                presenter.subNewBondStockRanking(ADDFLAG_ADDMORE, (currentPage + 1) * 20, 20, sortField, sortType);
                lsvContent.onRefreshComplete();
            }
        });
        ...
        toolBar.setToolBarClickListener(new ToolBar.ToolBarClickListener() {
            ...
            public void onClick(View view, ToolBar.Type type) {
                switch (type) {
                    case RIGHT_TYPE_1:
                        SearchHistoryActivity.startActivity(SubNewBondStockActivity.this);
                        break;
                }
            }
        });
    ...
    protected void initData() {
        ...
        presenter.subNewBondStockRanking(ADDFLAG_DEFAULT, 0, 20, sortField, TAG_UP);
    }
     ...
     */
    @Override
    public void subNewBondSuccess(ArrayList<SubNewStockRankingModel> result) {
        lsvContent.onRefreshComplete();
     ...
     */
    @Override
    public void subNewBondRefreshSuccess(ArrayList<SubNewStockRankingModel> result) {
        lsvContent.onRefreshComplete();
        if (result != null && result.size() > 0) {
            if(0 != currentPage) {
     ...
     */
    @Override
    public void subNewBondMoreSuccess(ArrayList<SubNewStockRankingModel> result) {
        lsvContent.onRefreshComplete();
        if (result != null && result.size() > 0) {
            currentPage = currentPage + 1;
            adapter.setData(result);
    ...
    public void getQuoteInfoSuccess(QuoteItem quoteItem) {
        ...
        StockDetailActivity.startActivity(SubNewBondStockActivity.this,
                quoteItems, 0);
    }
}
