...
public class HSBarChartView extends View{
...
//    private Paint mTextPaint = new Paint();
    ...
    private ArrayList<HSBarChartData> mData;
    ...
    public HSBarChartView(Context context) {
        super(context);
        init(context);
    }

    public HSBarChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public HSBarChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context){
        ...
        offset = SizeUtils.dp2px(context,1);
...
//        mTextPaint.setTextSize(SizeUtils.dp2px(context,8));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int width = mWidth/22;
        mRectWidth = SizeUtils.dp2px(mContext,mWidth /88f);
        ...
        for(int i = 0;i<mData.size();i++) {
            canvas.save();
            HSBarChartData hsbar = mData.get(i);

            startX = width * i + offset*(i+1);  // 每个小矩阵起点X
            //  startX = mWidth*i + offset;

            int color = hsbar.getmColor();
...
//            canvas.drawText(bar.getName(), startX, mHeight / 2, mNamePaint);
            canvas.drawText(hsbar.getName(),startX,(mHeight)/2-10, mNamePaint);
            if( !"0".equals(hsbar.getmValue()) ){
//                canvas.drawText(hsbar.getmValue(), startX, y, mTextPaint);
            }
        }
    }

    public void setData(ArrayList<HSBarChartData> datas){
...
//            initData();
    ...
    private void initData() {
        ...
        sum = 0;
        for(int i = 0;i<mData.size();i++){
            HSBarChartData data = mData.get(i);
        ...
        for(int i = 0;i<mData.size();i++){
            ...
            if(FormatUtils.isStandard(value)){
                data.setmHeight((Float.parseFloat(value)/sum) * mRatio*5);
            }
