...
public class VerticalBannerView extends LinearLayout implements BaseBannerAdapter.OnDataChangedListener {
    ...
    private void setupAdapter() {
        ...
        if(mAdapter.getCount() == 1){
            ...
            mAdapter.setItem(mFirstView,mAdapter.getItem(0),0);
            mFirstView.setTag(R.id.verticalbannerview_tag,0);
            mAdapter.setItem(mSecondView,mAdapter.getItem(1),1);
            mSecondView.setTag(R.id.verticalbannerview_tag,1);
            addView(mFirstView);
            addView(mSecondView);
            mPosition = 1;
            isStarted = false;
    ...
    @Override
    public void onChanged(boolean all) {
        if(all){
            setupAdapter();
        } else {
            ...
            for(int i = 0;i < count;i++){
                ...
                int position = 0;
                if(child.getTag(R.id.verticalbannerview_tag) != null){
                    position = (Integer) child.getTag(R.id.verticalbannerview_tag);
                }
                if(mAdapter.getCount()-1 > position){
                    mAdapter.setItem(child,mAdapter.getItem(position),position);
                }
            }
        }
    }
    ...
    private void performSwitch(){
        ...
        set.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                removedView.setTag(R.id.verticalbannerview_tag,mPosition%mAdapter.getCount());
