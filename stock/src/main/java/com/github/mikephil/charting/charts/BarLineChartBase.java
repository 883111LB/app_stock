...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
     ...
     */
    protected float mMinTopOffset = 15.f;

    /**
     * Sets the minimum offset (padding) around the chart, defaults to 15
     */
    protected float mMinBottomOffset = 15.f;
    ...
    public void setmMinBottomOffset(float mMinBottomOffset) {
        this.mMinBottomOffset  = Utils.convertDpToPixel(mMinBottomOffset);
    }

    public void setmMinTopOffset(float mMinTopOffset) {
        this.mMinTopOffset  = Utils.convertDpToPixel(mMinTopOffset);
    }

    public void setmMinLeftOffset(float mMinLeftOffset) {
        this.mMinLeftOffset  = Utils.convertDpToPixel(mMinLeftOffset);
    }

    public void setmMinRightOffset(float mMinRightOffset) {

        this.mMinRightOffset = Utils.convertDpToPixel(mMinRightOffset);
    }
    ...
    public void calculateOffsets() {
        ...
        if (!mCustomViewPortEnabled) {
            ...
            offsetBottom = Math.max(offsetBottom,minOffset);
            offsetLeft = Math.max(offsetLeft,minOffset);

            mViewPortHandler.restrainViewPort(
                    Math.min(mMinLeftOffset, offsetLeft),
                    Math.min(mMinTopOffset, offsetTop),
                    Math.min(mMinRightOffset, offsetRight),
                    Math.min(mMinBottomOffset, offsetBottom));
           /* mViewPortHandler.restrainViewPort(
                    3,
                    3,
                    3,
   ...
   /* *//**
    ...
    public float getTransX() {
        if (mViewPortHandler == null)
            ...
            return mViewPortHandler.getTransX();
    }
    /**
